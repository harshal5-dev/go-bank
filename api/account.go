...
import (
	...
	"github.com/go-bank/internal/token"
...
func (server *Server) getAccount(ctx *gin.Context) {
	...
	authPayload := ctx.MustGet(authorizationPayloadKey).(*token.Payload)
	if fetchedAccount.Owner != authPayload.Username {
...
func (server *Server) listAccount(ctx *gin.Context) {
	...
	fetchedAccounts, err := server.store.ListAccounts(ctx, arg)
	if err != nil {
	...
	ctx.JSON(http.StatusOK, fetchedAccounts)
}
